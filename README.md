...
Type a pattern, toggle `g i m s u y`, and matches light up in the test string as you type — adjacent ones alternate two tints behind a drawn edge, a zero-length one is a caret. The list beneath gives each its offset, text and groups. Matching runs in a Web Worker, so a runaway pattern dies at 400 ms; an invalid one becomes a readable line carrying the engine's message, cut in the middle past 136 characters and saying how much it dropped. A 35-entry cheat sheet inserts tokens or toggles flags. The count line names three limits — 1,000 matches, 100 rows, nothing run at 50,000 characters — and not a fourth: list text cut at 200.
